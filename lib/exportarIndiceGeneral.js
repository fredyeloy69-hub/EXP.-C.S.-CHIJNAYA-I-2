...
// Los nombres de carpeta en Drive ya vienen con su propia numeración manual
// pegada al nombre (ej. "1.1_ÍNDICE GENERAL...", "2.6. RELACION DE INSUMOS") —
// como ya generamos el N° SEPARADOR en su propia columna, quitamos ese prefijo
// del nombre para no repetir el número dos veces.
...
// mismo criterio de degradé que exportarExcel.js, para que la carpeta madre (nivel 1)
// sea la más nítida y cada nivel más profundo se vea más tenue.
...
export async function generarIndiceGeneralExcel(areaNombre, carpetasDelArea) {
  ...
  function agregarFilaSeparador(nombre, codigo, esSubEncabezado) {
    const nombreLimpio = limpiarNombre(nombre);
    const fila = sheet.addRow([null, codigo, nombreLimpio, null]);
    ...
    if (esSubEncabezado) {
      // Carpeta que a su vez contiene subcarpetas — sombreado tenue, cada vez
      // más pálido mientras más profundo el nivel (2, 3, 4, 5+).
      const nivel = codigo.split(".").length;
      const FACTORES_SUBNIVEL = [0.55, 0.72, 0.85, 0.92];
      const factor = FACTORES_SUBNIVEL[Math.min(nivel - 2, FACTORES_SUBNIVEL.length - 1)];
      const color = mezclarConBlanco(NARANJA, factor);
    ...
    fila.getCell(4).border = { top: BORDE_GRIS, bottom: BORDE_GRIS, left: BORDE_GRIS, right: BORDE_GRIS };
    return fila;
  }

  function agregarFilaOrden(nombre, numero, archivadores) {
    const nombreLimpio = limpiarNombre(nombre);
    const fila = sheet.addRow([numero, null, nombreLimpio, archivadores || null]);
  ...
  function agruparRecursivo(items, nivelIdxRuta, codigoPadre) {
    ...
    if (!hayNivelMasProfundo) {
      ...
      ordenado.forEach((c, i) => {
        agregarFilaSeparador(c.nombre || (c.ruta || "").split(" / ").pop() || "-", `${codigoPadre}.${i + 1}`, false);
      });
      return;
    }
    ...
    entradas.forEach((entrada, i) => {
      ...
      if (entrada.esGrupo) {
        agregarFilaSeparador(entrada.key, codigo, true);
        ...
        ordenado.forEach((c) => {
          agregarFilaSeparador(c.nombre, codigo, false);
        });
      }
    });
  }
